/* randn_complex_5
  ...
  Using pointer arithmetic, in 1-D for 2D complex double with
  complex double *ptr;

  compile for gcc-5:

gcc -std=gnu11 -Wall -Ofast -msse2 -frename-registers -malign-double -fno-strict-aliasing -DHAVE_SSE2=1 -DDSFMT_MEXP=19937 -o randn_complex_5 dSFMT.c randn_complex_5.c -lm -lrt -lgsl -lgslcblas
...
inline static void randn_box_muller(long n, complex double r[][n])
{
   ...
   register long i;
   complex double *ptr;
   ptr = r;  //2D pointer complex [n][n] address to 1D complex array pointer [n*n]
...
inline static void randn_box_SIMD(long n,complex double r[][n])
{
   ...
   for(i = 0; i < n*n; i++){
      ...
      *ptr++ = (fast_sin(x) + I*fast_cos(x))*(double)yout;
  }
  return;
}
