...
inline static void randn_polar(double *u, double *v){
  register double s, x, y;    // Marsaglia polar using both u and v for complex return
    for(;;){
...
inline static void randn_box_muller_fast(double *u, double *v)
{
   register double x, y;
      x = 2.*M_PI*dsfmt_genrand_close_open(&dsfmt);
      y = sqrt(-2.*log(dsfmt_genrand_close_open(&dsfmt)));
      *u = y*fast_sin(x);*v = y*fast_cos(x);
  return;
}

inline static void randn_box_muller(double *u, double *v)
{
   register double x, y;
      x = 2.*M_PI*dsfmt_genrand_close_open(&dsfmt);
      y = sqrt(-2.*log(dsfmt_genrand_close_open(&dsfmt)));
...
inline static void randn_box_SIMD(double *u, double *v)
{
;
  register double x;
  float yin, yout;
  ...
  *u = (double)yout*fast_cos(x);
  *v = (double)yout*fast_sin(x);
  return;
}
